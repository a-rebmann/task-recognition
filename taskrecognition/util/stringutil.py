...
from taskrecognition.const import TERMS_FOR_MISSING
...
def clean_attributes(attributes) -> tuple[str]:
    return tuple(preprocess_label(att) for att in attributes if not pd.isna(att) and att not in TERMS_FOR_MISSING)