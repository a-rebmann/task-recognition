...
from taskrecognition.util.csv import log_converter
from taskrecognition.const import *
...
def prepeare_log(log_name, parse_dates=False):
    if ".xes" in log_name:
        ...
        df = pd.read_csv(path_to_files + "/" + log_dir + "/" + log_name, sep=",", quotechar='"', engine="python", parse_dates=parse_dates, on_bad_lines='skip')
    return df
...
def buildMergedLogAgostinelli(num_of_logs=10):
    for num_of_log in range(num_of_logs):
        ...
        for index, row in df2.iterrows():
            if log2_prev is not None and log2_prev != row[
                "caseID"]:  # == "https://submit.jotform.com/submit/200477494954062/" and row["eventType"] == "clickLink":
        ...
        for index, row in df3.iterrows():
            ...
            if log3_prev is not None and log3_prev != row[
                "caseID"]:  # "https://forms.zoho.com/universityofmelbourne/form/NewRecord/thankyou":
...
###################################
# Uncomment the line below to create random evaluation logs based on available task data
###################################
